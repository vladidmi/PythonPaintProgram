...
draw_structure_buttons = {
    CONCRETE:Button(y=button_y, width=BOX_SIZE, height=BOX_SIZE, color=all_colors[CONCRETE], text = CONCRETE, text_color= WHITE, label = CONCRETE),
    PREFABRICATED_PART:Button(y=button_y, width=BOX_SIZE, height=BOX_SIZE, color=all_colors[PREFABRICATED_PART], text = PREFABRICATED_PART, text_color= WHITE, label = PREFABRICATED_PART),
    MASONRY:Button(y=button_y, width=BOX_SIZE, height=BOX_SIZE, color=all_colors[MASONRY], text = MASONRY, text_color= WHITE, label = MASONRY),
}
...
plan_buttons = {
    FORMWORK:Button(y=button_y, width=BOX_SIZE, height=BOX_SIZE, color=all_colors[FORMWORK], text = FORMWORK, label = FORMWORK),
    REINFORCE:Button(y=button_y, width=BOX_SIZE, height=BOX_SIZE, color=all_colors[REINFORCE], text = REINFORCE, label = REINFORCE),
    POUR_CONCRETE:Button(y=button_y, width=BOX_SIZE, height=BOX_SIZE, color= all_colors[POUR_CONCRETE], text = POUR_CONCRETE, label = POUR_CONCRETE),
    PREFABRICATED_PART_ASSEMBLE:Button(y=button_y, width=BOX_SIZE, height=BOX_SIZE, color=all_colors[PREFABRICATED_PART_ASSEMBLE], text = PREFABRICATED_PART_ASSEMBLE, label = PREFABRICATED_PART_ASSEMBLE),
    DO_MASONRY:Button(y=button_y, width=BOX_SIZE, height=BOX_SIZE, color=all_colors[DO_MASONRY], text = DO_MASONRY, label = DO_MASONRY),
    PART_COMPLETE:Button(y=button_y, width=BOX_SIZE, height=BOX_SIZE, color= all_colors[PART_COMPLETE], text = PART_COMPLETE, label = PART_COMPLETE),    
}
...
tact_button_colors = []
tact_buttons = {
    f'{TACT_PART} {i+1}': Button(y=button_y, width=BOX_SIZE, height=BOX_SIZE, color=all_colors[f'{TACT_PART} {i+1}'], text = f'{TACT_PART} {i+1}', label = f'{TACT_PART} {i+1}') 
